...
bool Options::parseCommandLine(int argc, const char *const *argv)
{
    ...
    desc_.add_options()(
        ...
        FileFormat::isSupported(FileFormat::Opus) ? "input file name (.mp3, .wav, .flac, .ogg, .oga, .opus, .dat)" : "input file name (.mp3, .wav, .flac, .ogg, .oga, .dat)")(
        "output-filename,o",
        ...
        FileFormat::isSupported(FileFormat::Opus) ? "input file format (mp3, wav, flac, ogg, opus, dat)" : "input file format (mp3, wav, flac, ogg, dat)")(
        "network-file,n",
        po::value<std::string>(&url_),
        "network file URL")(
        "output-format",
    ...
    try
    {
        ...
        if (input_filename_.empty() && input_format_.empty() && url_.empty())
        {
